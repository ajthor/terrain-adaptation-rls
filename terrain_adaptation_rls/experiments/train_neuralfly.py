"""Direct NeuralFly-style learned-basis training command."""
...
DEFAULT_CONFIG = "configs/train/warty_neuralfly_scaled.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=f"Path to a NeuralFly-style train config. Defaults to {DEFAULT_CONFIG}.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    _require_neuralfly_style(config, Path(args.config))

    if args.dry_run:
        print(
            f"valid NeuralFly-style train config: {config.name} "
...
def _require_neuralfly_style(config: ExperimentConfig, path: Path) -> None:
    family = str(config.model.get("family", ""))
    if family not in {"neuralfly_style", "neuralfly"}:
        raise ValueError(
            f"{path} is not a NeuralFly-style config; "
