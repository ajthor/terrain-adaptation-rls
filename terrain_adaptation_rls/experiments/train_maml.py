"""Direct MAML Neural ODE training command."""
...
DEFAULT_CONFIG = "configs/train/warty_maml_debug.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=f"Path to a MAML train config. Defaults to {DEFAULT_CONFIG}.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    _require_maml(config, Path(args.config))

    if args.dry_run:
        print(
            f"valid MAML train config: {config.name} "
...
def _require_maml(config: ExperimentConfig, path: Path) -> None:
    family = str(config.model.get("family", ""))
    if family not in {"maml", "maml_neural_ode"}:
        raise ValueError(
            f"{path} is not a MAML config; "
