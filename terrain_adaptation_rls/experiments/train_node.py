"""Direct static Neural ODE training command."""
...
DEFAULT_CONFIG = "configs/train/warty_node_scaled.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=f"Path to a static Neural ODE train config. Defaults to {DEFAULT_CONFIG}.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    _require_neural_ode(config, Path(args.config))

    if args.dry_run:
        print(
            f"valid NODE train config: {config.name} "
            f"({config.platform}, hidden={config.model.get('hidden_size', 128)})"
        )
        return 0
...
def _require_neural_ode(config: ExperimentConfig, path: Path) -> None:
    family = str(config.model.get("family", ""))
    if family not in {"neural_ode", "node"}:
        raise ValueError(
            f"{path} is not a static Neural ODE config; "
