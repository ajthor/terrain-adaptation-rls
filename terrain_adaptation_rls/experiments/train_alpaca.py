"""Direct ALPaCA learned-prior training command."""
...
DEFAULT_CONFIG = "configs/train/warty_alpaca_scaled.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=f"Path to an ALPaCA train config. Defaults to {DEFAULT_CONFIG}.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    _require_alpaca(config, Path(args.config))

    if args.dry_run:
        print(
            f"valid ALPaCA train config: {config.name} "
    ...
    prepared = prepare_run(
        args.config,
        command="train_alpaca",
        run_name=args.run_name,
    )
    from terrain_adaptation_rls.training.alpaca import run_alpaca_training

    metrics = run_alpaca_training(
        prepared.config,
        device=args.device,
...
def _require_alpaca(config: ExperimentConfig, path: Path) -> None:
    family = str(config.model.get("family", ""))
    if family != "alpaca":
        raise ValueError(
            f"{path} is not an ALPaCA config; model.family is '{family or '<missing>'}'"
        )
